...
SCHEME_JSON = "data/processed/schemes.json"
...
def startup():
    ...
    # ---------- FAST SCHEME CACHE ----------
    if os.path.exists(SCHEME_JSON):
        print("⚡ Loading cached schemes.json")
        with open(SCHEME_JSON, "r") as f:
            SCHEME_CACHE = json.load(f)
    else:
        print("🧮 Building scheme cache...")
        schemes_df = get_all_schemes(valid_master_db)

        SCHEME_CACHE = schemes_df.to_dict("records")

        os.makedirs("data/processed", exist_ok=True)
        with open(SCHEME_JSON, "w") as f:
            json.dump(SCHEME_CACHE, f)

        print("💾 schemes.json created")
...
def lumpsum_yearly(scheme_code: int, amount: float, start_date: str, end_date: str):
    return get_lumpsum_yearwise_growth(
...
def sip_yearly(scheme_code: int, monthly_amount: float, start_date: str, end_date: str):
    return get_sip_yearwise_growth(
